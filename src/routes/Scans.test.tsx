  ...
  it("counts what each scan finds and leads into the screener with its conditions", async () => {
    stubPlatform({
      ...
      "/api/screen": {
        bodyFor: (path: string) => ({
          as_of: "2026-09-22",
          // Oversold finds 12; everything else 3.
          total: path.includes("rsi%3Alt%3A30") ? 12 : 3,
          limit: 1,
          offset: 0,
          items: [],
        }),
    ...
    expect(await within(momentum).findByText("RSI (14) < 30")).toBeInTheDocument();
    const run = within(momentum).getAllByRole("link", { name: /Run in the screener/ })[0];
    expect(run).toHaveAttribute("href", "/screen?where=rsi%3Alt%3A30");
  });
